    ...
    namespace SewaRuanganUmy2
    {
        ...
        private void btnTambahkan_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_InsertReservasi", con, trans))
                    {
                ...
                {
                    trans.Rollback();
                    MessageBox.Show("Gagal menambahkan reservasi: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void LoadData()
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                string query = "SELECT id_reservasi, id_pelanggan, id_paket, id_ruangan, tanggal_reservasi, jam_mulai, jam_selesai, status FROM Reservasi ORDER BY tanggal_reservasi DESC";
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (txtJamMulai.Value.TimeOfDay >= txtJamSelesai.Value.TimeOfDay)
            {
                MessageBox.Show("Jam selesai harus setelah jam mulai.", "Validasi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_UpdateReservasi", conn, trans))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
